...
```text
TutorHub/
├── docs/                           # Tài liệu chi tiết dự án (API, Schema, PRD)
│   ├── api-design.md
│   ├── database-schema.md
│   └── prd.md
├── src/
...
│   ├── TutorHub.Infrastructure/    # EF Core DbContext, PostgreSQL, JWT, VNPay, Background Jobs
│   └── TutorHub.Api/               # REST API Controllers, Middlewares, Configurations
├── docker-compose.yml              # Cấu hình khởi chạy Docker PostgreSQL & API
...
#### Bước 1: Cấu hình chuỗi kết nối Database trong `src/TutorHub.Api/appsettings.json`
```json
"ConnectionStrings": {
  "DefaultConnection": "Host=localhost;Port=5432;Database=tutorhub;Username=tutorhub;Password=tutorhub"
}
```
...
Chạy script `seedData.sql` vào cơ sở dữ liệu PostgreSQL của bạn để tạo sẵn các tài khoản mẫu:
- **Admin:** `admin@tutorhub.com` / `Admin123!`
- **Gia Sư:** `tutor@tutorhub.com` / `Tutor123!`
- **Học Viên:** `student@tutorhub.com` / `Student123!`
