...
export default function ServiceSyllabus({ curriculum, totalSessions, sessionDurationMinutes }) {
  ...
  return (
    <div id="syllabus" className="bg-white rounded-2xl border border-neutral-200 p-6 sm:p-8 shadow-2xs">
      {/* Header Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-neutral-100">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Tổng cộng <strong className="text-slate-800">{totalSessions || sessions.length} buổi học</strong> •{' '}
            <strong className="text-slate-800">{totalHours} giờ học</strong> chuyên sâu cá nhân hóa.
          </p>
        </div>
        ...
        {sessions.length > 0 && (
          <button
            type="button"
            onClick={toggleAll}
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-blue-600 hover:text-blue-700 bg-blue-50/80 hover:bg-blue-100/80 px-3.5 py-2 rounded-xl transition-colors self-start sm:self-auto"
          >
          ...
          return (
            <div
              ...
              className={`rounded-xl border transition-all duration-200 overflow-hidden ${
                isOpen
                  ? 'border-blue-300 bg-gradient-to-b from-blue-50/30 to-white shadow-xs'
            ...
            >
              ...
              >
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <span className="hidden sm:inline-flex items-center gap-1 text-xs text-slate-500 font-medium bg-slate-50 px-2.5 py-1 rounded-md border border-neutral-100">
                    <Icon name="schedule" size="xs" className="w-3.5 h-3.5 text-slate-400" />
              ...
              {/* Accordion Body */}
              {isOpen && (
                <div className="px-5 pb-5 pt-1 border-t border-blue-100/60 flex flex-col gap-3.5 animate-fadeIn">
                  {item.description && (
