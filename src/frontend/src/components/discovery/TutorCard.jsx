...
export default function TutorCard({ tutor, onMessageClick }) {
  ...
  return (
    <article className="group bg-white rounded-[20px] border border-neutral-200/80 hover:border-brand-primary-300 hover:shadow-brand-md transition-all duration-200 flex flex-col justify-between p-5 relative">
      <div>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                <Link
                  to={`/tutors/${id}`}
                  className="font-semibold text-[16px] text-neutral-900 hover:text-[#2563EB] transition-colors truncate"
                  title={fullName}
                >
                  {fullName}
                </Link>
          ...
          <button
            ...
            className="p-1.5 rounded-full text-neutral-400 hover:text-rose-500 hover:bg-rose-50 transition-colors cursor-pointer shrink-0 -mr-1"
            aria-label={isFavorite ? 'Xóa khỏi yêu thích' : 'Lưu vào yêu thích'}
          >
            <Icon
              name="favorite"
              size="sm"
              className={cn(isFavorite ? 'text-rose-500 fill-rose-500' : 'text-neutral-400')}
            />
          </button>
        </div>
        ...
        <div className="flex items-center gap-1.5 mt-3 text-[13px] text-neutral-600 whitespace-nowrap overflow-hidden">
          <span className="flex items-center gap-1 text-amber-500 font-bold shrink-0">
            <Icon name="star" size="xs" className="fill-amber-400 text-amber-400 w-4 h-4" />
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-3">
          ...
          {subjects.length > 2 && (
            <span className="px-2 py-0.5 rounded-md bg-blue-50 text-[#2563EB] text-[11px] font-semibold">
              +{subjects.length - 2}
            </span>
          )}
      ...
      {/* 6 & 7. Footer: Giá (dùng màu #0F172A, không dùng green) + [Nhắn tin] [Xem hồ sơ →] */}
      <div className="pt-3.5 mt-3.5 border-t border-neutral-100 flex items-center justify-between gap-2">
        <div className="shrink-0 min-w-0">
          ...
          <div className="text-[16px] font-bold text-[#0F172A] font-mono tracking-tight leading-tight mt-0.5">
            {hourlyRate > 0 ? (
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Link
            to={`/tutors/${id}`}
            className="inline-flex items-center gap-1 px-3.5 py-1.5 rounded-[8px] bg-[#2563EB] hover:bg-[#1D4ED8] active:bg-[#1E40AF] text-white text-[12.5px] font-medium whitespace-nowrap cursor-pointer transition-colors shadow-sm"
          >
            <span>Xem hồ sơ</span>
