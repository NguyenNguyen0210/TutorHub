...
public class ConfirmBookingCommandHandler : IRequestHandler<ConfirmBookingCommand, BookingDto>
{
    ...
    public ConfirmBookingCommandHandler(IAppDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<BookingDto> Handle(ConfirmBookingCommand request, CancellationToken cancellationToken)
    {
        ...
        // 1. Resource Ownership Check (Only the designated Tutor can confirm)
        ...
        {
            throw new ConflictException($"Cannot confirm booking in '{booking.Status}' status.");
        }

        // 3. Check 24-hour confirmation window
        if (now > booking.CreatedAt.AddHours(24))
        {
            // Auto expire & full refund
            ...
            throw new BadRequestException("The 24-hour confirmation window has expired. The booking has been cancelled and refunded to the student.");
        }
